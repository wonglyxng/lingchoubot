...
var promptVersions = map[string]PromptVersion{
	"pm":         {Role: "pm", Version: "1.1.2", ChangeLog: "强化项目范围约束：除目标绑定外，还要求严格遵守仅前端、无后端、无 CMS 等显式非目标"},
	"supervisor": {Role: "supervisor", Version: "1.2.4", ChangeLog: "默认评审模板由系统根据 task_category 自动解析，避免冗余 review_template_key 造成契约失败"},
...
- 任务标题和描述必须复用项目名称/描述中的核心业务名词，不要替换成其他业务域；如果项目是计算器，就不要擅自改写成宠物领养、CMS、博客等其他系统
- 如果项目明确写了“仅前端”或“不包含后端 / CMS / 后台管理”等限制，必须把这些内容视为非目标；不要输出后端 API、CMS、后台管理、内容发布等越界任务
...
- 不要输出 review_template_key，系统会根据 task_category 自动解析默认模板并写入契约 metadata
- 默认情况下不要输出 review_policy，系统会直接使用 task_category 对应的默认模板
- 只有任务存在明确专项要求时，才输出 review_policy 作为默认模板上的 override
- 如果输出 review_policy，必须同时输出 review_policy_reason 和 review_policy_source
- review_policy_reason 必须明确说明“为什么默认模板不够，专项要求是什么”
- review_policy_source 必须列出专项要求依据来自哪里，例如 task.description、acceptance_criteria、project constraints、user instruction
- 如果输出 review_policy，不要原样复制整套默认模板
...
- 如果输出 review_policy，其中最终 score_items 权重总和必须仍然满足 100 分
