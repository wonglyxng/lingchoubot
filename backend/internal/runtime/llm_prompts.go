...
var promptVersions = map[string]PromptVersion{
	...
	"supervisor": {Role: "supervisor", Version: "1.2.0", ChangeLog: "契约输出增加任务类别与评审策略字段，便于后续结构化评审"},
  ...
  "contracts": [
    {
      ...
      "review_policy": {
        "pass_threshold": 80,
        "hard_gates": [{"key": "goal_match", "name": "工件与任务目标一致"}],
        "score_items": [{"key": "completeness", "name": "完整性", "weight": 25}]
      }
    }
  ],
  "assignments": [
...
- review_policy 必须至少包含 pass_threshold、hard_gates、score_items 三部分
