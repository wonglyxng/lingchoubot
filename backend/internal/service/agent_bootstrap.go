	...
	return []BaselineAgentSpec{
		...
		},
		{
			Agent: model.Agent{
				...
				ManagedRoles:   model.JSON(`["GENERAL_WORKER","BACKEND_DEV_WORKER","FRONTEND_DEV_WORKER"]`),
				AllowedTools:   model.JSON(`[]`),
				Capabilities:   model.JSON(`{}`),
				Metadata:       defaultBaselineAgentMetadata(),
			},
		...
		},
		{
			Agent: model.Agent{
				...
				RoleCode:       model.RoleCodeGeneralWorker,
				AgentType:      model.AgentTypeLLM,
				Status:         model.AgentStatusActive,
				Specialization: model.AgentSpecGeneral,
				Description:    "通用分析执行 Agent，负责需求梳理、可行性评估与文档类交付。",
				ManagedRoles:   model.JSON(`[]`),
				AllowedTools:   model.JSON(`["artifact_storage"]`),
				Capabilities:   model.JSON(`{}`),
				Metadata:       defaultBaselineAgentMetadata(),
			},
