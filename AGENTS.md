...
采用 monorepo。项目结构示意统一维护在 README 中，此处只保留结构约束描述。

当前仓库按“单仓库、多模块、多可执行入口、可独立部署”组织，后端与前端共用同一仓库，但可以在运行态拆分为不同进程或不同机器部署。
# 1. 项目概述

灵筹是一个**面向复杂项目交付的多智能体组织操作系统**。
核心链路：`项目 -> 阶段 -> 任务 -> 工件 -> 审批 -> 审计`。
3. 后端新增可执行入口应优先放在 `backend/cmd/` 下

当前处于 **MVP 第一阶段**，目标是做一个**最小可运行闭环**，而不是完整 AI 公司。
5. 部署形态可以拆分，但仓库结构仍按 monorepo 统一管理
...
2. 新模块必须放入合理位置
3. 后端新增可执行入口应优先放在 `backend/cmd/` 下
4. migration 必须单独存放在 `backend/migrations/`
5. 文档必须放到 `docs/`
6. 部署形态可以拆分，但仓库结构仍按 monorepo 统一管理
